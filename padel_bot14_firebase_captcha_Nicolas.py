...
# ─────────────────────────────────────────────
CONFIG = {
    "email":    os.environ.get("PADEL_EMAIL1", ""),
    "password": os.environ.get("PADEL_PASSWORD1", ""),
...
async def reserve_slots(page: Page, date_str: str, db=None, dry_run: bool = False):
    ...
    slots_result = await page.evaluate("""
        () => {
            const divs = document.querySelectorAll('div.heure');
            // Libellés de terrain : n'importe quel élément court contenant "terrain",
            // avec une hauteur de ligne de texte (pas un gros conteneur qui l'englobe).
            const terrainLabels = [];
            document.querySelectorAll('*').forEach(el => {
                const t = el.textContent.trim();
                if (/terrain/i.test(t) && t.length > 0 && t.length < 80) {
                    const rect = el.getBoundingClientRect();
                    if (rect.height > 0 && rect.height < 80) {
                        terrainLabels.push({ text: t, top: rect.top + window.scrollY, height: rect.height });
                    }
                }
            });
            ...
            return {
                slots: result,
                debug_labels: terrainLabels.map(t => t.text + ' @top=' + Math.round(t.top) + ' h=' + Math.round(t.height))
            };
        }
    """)
    slots_info = slots_result["slots"]
    log.info(f"Libellés terrain détectés (diagnostic) : {slots_result['debug_labels']}")
    ...
    if not slots_to_book:
        ...
        results.append(("N/A", "aucun_creneau_disponible", "", CONFIG.get("email", "")))
        return 0, results
    ...
    for slot in slots_to_book:
        if dry_run:
            ...
            reserved += 1
            write_reservation_to_firebase(db, date_str, slot['time'], "dry_run", slot.get("terrain", ""), CONFIG.get("email", ""))
            results.append((slot['time'], "dry_run", slot.get("terrain", ""), CONFIG.get("email", "")))
            continue
        ...
        try:
            ...
            if valider:
                ...
                if "validée" in page_text.lower() or "réservation validée" in page_text.lower():
                    ...
                    results.append((slot['time'], "confirmée", terrain_name, email_used))
                    ...
                    results.append((slot['time'], "echec_captcha", terrain_name, email_used))
                else:
                    ...
                    results.append((slot['time'], "probable", terrain_name, email_used))
            else:
                log.warning(f"Valider ma réservation non trouvé pour {slot['time']}")
                write_reservation_to_firebase(db, date_str, slot['time'], "echec_validation", slot.get("terrain", ""), CONFIG.get("email", ""))
                results.append((slot['time'], "echec_validation", slot.get("terrain", ""), CONFIG.get("email", "")))
            ...
            write_reservation_to_firebase(db, date_str, slot['time'], f"erreur: {str(e)[:50]}", slot.get("terrain", ""), CONFIG.get("email", ""))
            results.append((slot['time'], f"erreur: {str(e)[:50]}", slot.get("terrain", ""), CONFIG.get("email", "")))
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if nb_to_reserve <= 0:
                ...
                status_lines.extend(
                    f"  {heure} → {statut} — Terrain : {terrain or 'inconnu'} — Compte : {email}"
                    for heure, statut, terrain, email in slot_results
                )
